...
export default function NewProductionPage() {
  ...
  function validateDetails() {
    return (
      ...
      details.eventDate.length > 0 &&
      details.startTime.length > 0
    );
  }
  ...
  function validateSegments() {
    return (
      segments.length > 0 &&
      segments.every((s) => s.title.trim().length > 0)
    );
  }
      ...
      {step === 1 ? (
        <Card className="shadow-sm">
          ...
          <CardContent>
            ...
            <FormActions
              ...
              onSubmit={() => validateDetails() && setStep(2)}
              submitLabel="Continue to Run Sheet"
              submitType="button"
              submitDisabled={!validateDetails()}
            />
          </CardContent>
        </Card>
      ) : (
        <div className="flex flex-col gap-6">
          ...
          <FormActions
            ...
            submitType="button"
            isLoading={isSaving}
            submitDisabled={!validateSegments()}
          />
        </div>
      )}
    </div>
